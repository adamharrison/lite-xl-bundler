...
lite_xls = JSON.parse(`./lpm lite-xl list --json`)["lite-xls"].select { |x| x['status'] == 'available' }.map { |x|
  "<option value='#{x["version"]}'>#{x["version"]}</option>"
}.join("\n")
has_plugin = {}
plugins = JSON.parse(`./lpm plugin list --json`)["plugins"].sort { |a,b| (a["name"] || a["id"]) + b["version"] <=> (b["name"] || b["id"]) + a["version"] }.map { |x|
  if !has_plugin[x["id"]] 
    has_plugin[x["id"]] = 1
